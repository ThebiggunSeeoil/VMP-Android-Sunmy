...
export const GuardQrPassRequestScreen: React.FC<{ navigation: any }> = ({ navigation }) => {
  const { guardhouse, guard, showPhotoPreview, enableGateControl } = useAppStore();
  ...
  const submit = async (capturedPlatePhoto = platePhoto) => {
    if (!requestType || !reason || !house || !idPhoto || !capturedPlatePhoto || !guardhouse?.serviceId) return;
    setLoading(true);
    try {
      const response = await vmsApi.submitGuardQrPassRequest({
        ...
        picture_id_card: idPhoto, picture_car_number: capturedPlatePhoto,
      });
  ...
  const checkInAndPrint = async () => {
    ...
    try {
      ...
      if (printed?.success && enableGateControl) {
  ...
  const openEntryGate = async () => {
    setShowGateModal(false);
    setLoading(true);
    try {
      const response = await vmsApi.sendGateCommand(
        'door_open_in',
        guardhouse?.id,
        guardhouse?.serviceId,
        guard?.userId,
        {
          houseNo: house,
          triggerSource: 'guard_qr_pass_request',
          guardName: guard?.name,
          guardhouseName: guardhouse?.name,
          serviceName: guardhouse?.villageName || guardhouse?.name,
        }
      );
      const success = response?.success === true || response?.status === true || response?.message === 'Command sent successfully';
      setResultModal(success
        ? { type: 'success', title: 'เปิดไม้กั้นสำเร็จ', message: 'ส่งสัญญาณเปิดไม้กั้นขาเข้าเรียบร้อยแล้ว' }
        : { type: 'error', title: 'เปิดไม้กั้นไม่สำเร็จ', message: response?.message || 'ไม่สามารถส่งคำสั่งเปิดไม้กั้นได้' }
      );
    } catch (error: any) {
      setResultModal({ type: 'error', title: 'เปิดไม้กั้นไม่สำเร็จ', message: error?.message || 'ไม่สามารถส่งคำสั่งเปิดไม้กั้นได้' });
    } finally { setLoading(false); }
  };
  ...
  return <View style={s.root}>
    ...
    </View><Text style={s.autoSubmitText}>กำลังส่งคำขอออกบัตร QR Code อัตโนมัติ...</Text></View>}
    </ScrollView>
...
const CheckInPromptModal = ({ visible, houseNo, reasonName, onFinish, onConfirm }: { visible: boolean; houseNo: string; reasonName: string; onFinish: () => void; onConfirm: () => void }) => (
  <Modal visible={visible} transparent animationType="fade" statusBarTranslucent onRequestClose={onFinish}>
    <View style={s.modalBackdrop}><View style={s.modalCard}><View style={s.modalIcon}><Text style={s.modalIconText}>✓</Text></View><Text style={s.modalTitle}>ส่งคำขอบัตร QR Code สำเร็จ</Text><View style={s.requestDetail}><Text style={s.requestDetailLabel}>บ้านเลขที่</Text><Text style={s.requestDetailValue}>{houseNo === '000/00' ? '000/00 - ผู้มาติดต่อส่วนกลาง' : houseNo}</Text><Text style={s.requestDetailLabel}>เหตุผลในการเข้าออก</Text><Text style={s.requestDetailValue}>{reasonName}</Text></View><Text style={s.modalMessage}>ต้องการพิมพ์บัตรผ่านสำหรับการเข้าครั้งนี้หรือไม่</Text><View style={s.modalActions}><TouchableOpacity style={s.modalSecondary} onPress={onFinish}><Text style={s.modalSecondaryText}>จบขั้นตอน</Text></TouchableOpacity><TouchableOpacity style={s.modalPrimary} onPress={onConfirm}><Text style={s.modalPrimaryText}>พิมพ์บัตรผ่านเฉพาะครั้งนี้</Text></TouchableOpacity></View></View></View>
  </Modal>
);

const s = StyleSheet.create({
  root:{flex:1,backgroundColor:'#F1F5F9'},subHeader:{height:58,backgroundColor:'#FFF',borderBottomWidth:1,borderBottomColor:'#E2E8F0',flexDirection:'row',alignItems:'center',justifyContent:'space-between',paddingHorizontal:14},backBtn:{width:100},back:{color:'#2563EB',fontSize:15,fontWeight:'800'},subHeaderTitle:{color:'#0F172A',fontSize:16,fontWeight:'900',textAlign:'center'},subHeaderSpacer:{width:100},content:{padding:14,paddingBottom:106},progress:{color:'#2563EB',fontSize:14,fontWeight:'800',marginTop:6,marginBottom:14},section:{gap:10},heading:{color:'#0F172A',fontSize:20,fontWeight:'900'},caption:{color:'#64748B',fontSize:13,lineHeight:19},choice:{minHeight:96,flexDirection:'row',alignItems:'center',backgroundColor:'#FFF',borderWidth:2,borderColor:'#BFDBFE',borderLeftWidth:5,borderLeftColor:'#2563EB',borderRadius:14,padding:14,shadowColor:'#1D4ED8',shadowOffset:{width:0,height:2},shadowOpacity:0.08,shadowRadius:5,elevation:2},choiceIcon:{fontSize:29,marginRight:12},choiceCopy:{flex:1},choiceTitle:{color:'#0F172A',fontSize:18,fontWeight:'900'},choiceHint:{color:'#64748B',fontSize:13,fontWeight:'700',marginTop:3,lineHeight:18},arrow:{color:'#2563EB',fontSize:20,fontWeight:'900',lineHeight:22},reasonHeader:{flexDirection:'row',alignItems:'center',gap:8},stepNumber:{width:26,height:26,borderRadius:13,alignItems:'center',justifyContent:'center',backgroundColor:'#1D4ED8'},stepNumberText:{color:'#FFF',fontSize:14,fontWeight:'900'},reasonList:{gap:10,marginTop:3},reasonCard:{flexDirection:'row',alignItems:'center',backgroundColor:'#FFF',paddingVertical:12,paddingHorizontal:12,borderRadius:14,borderWidth:2,borderLeftWidth:5,borderColor:'#BFDBFE',borderLeftColor:'#2563EB',shadowColor:'#1D4ED8',shadowOffset:{width:0,height:2},shadowOpacity:0.08,shadowRadius:5,elevation:2},reasonNumBadge:{width:32,height:32,borderRadius:11,justifyContent:'center',alignItems:'center',backgroundColor:'#EFF6FF',borderWidth:1.5,borderColor:'#93C5FD',marginRight:9},reasonNumText:{fontSize:14,fontWeight:'900',color:'#1D4ED8'},reasonIconBox:{width:44,height:44,borderRadius:12,justifyContent:'center',alignItems:'center',backgroundColor:'#EFF6FF',borderWidth:1.5,borderColor:'#DBEAFE',marginRight:10},reasonEmoji:{fontSize:22},reasonTextBox:{flex:1,justifyContent:'center'},reasonTitle:{color:'#0F172A',fontSize:15,fontWeight:'900'},houseRequiredTag:{alignSelf:'flex-start',marginTop:4,paddingHorizontal:6,paddingVertical:2,borderRadius:4,backgroundColor:'#FEE2E2'},houseRequiredTagText:{color:'#B91C1C',fontSize:10,fontWeight:'800'},houseSkippableTag:{alignSelf:'flex-start',marginTop:4,paddingHorizontal:6,paddingVertical:2,borderRadius:4,backgroundColor:'#FEF3C7'},houseSkippableTagText:{color:'#A16207',fontSize:10,fontWeight:'800'},houseOptionalTag:{alignSelf:'flex-start',marginTop:4,paddingHorizontal:6,paddingVertical:2,borderRadius:4,backgroundColor:'#DCFCE7'},houseOptionalTagText:{color:'#15803D',fontSize:10,fontWeight:'800'},reasonArrowBadge:{width:28,height:28,borderRadius:14,justifyContent:'center',alignItems:'center',backgroundColor:'#EFF6FF',marginLeft:7},reasonArrow:{color:'#2563EB',fontSize:20,fontWeight:'900',lineHeight:22},empty:{color:'#64748B',fontSize:14,textAlign:'center',marginTop:36},photoStep:{alignItems:'center',paddingTop:48},bigIcon:{fontSize:52,marginBottom:16},primary:{backgroundColor:'#1D4ED8',borderRadius:8,paddingVertical:14,paddingHorizontal:20,marginTop:24},autoSubmitText:{color:'#047857',fontSize:14,fontWeight:'800',textAlign:'center',marginTop:4},buttonText:{color:'#FFF',fontSize:16,fontWeight:'900',textAlign:'center'},summary:{backgroundColor:'#FFF',borderWidth:1.5,borderColor:'#BFDBFE',borderRadius:12,padding:16,gap:7},summaryLabel:{color:'#64748B',fontSize:12,fontWeight:'800',marginTop:4},summaryValue:{color:'#0F172A',fontSize:16,fontWeight:'800',marginTop:2},modalBackdrop:{flex:1,backgroundColor:'rgba(15,23,42,0.78)',justifyContent:'center',alignItems:'center',padding:24},modalCard:{width:'100%',maxWidth:340,backgroundColor:'#FFF',borderRadius:24,padding:24,alignItems:'center',borderWidth:1,borderColor:'#E2E8F0'},modalIcon:{width:64,height:64,borderRadius:32,justifyContent:'center',alignItems:'center',backgroundColor:'#DCFCE7',borderWidth:2,borderColor:'#86EFAC',marginBottom:14},modalIconText:{color:'#16A34A',fontSize:32,fontWeight:'900'},modalTitle:{color:'#0F172A',fontSize:21,fontWeight:'900',textAlign:'center'},requestDetail:{width:'100%',backgroundColor:'#F8FAFC',borderWidth:1,borderColor:'#DBEAFE',borderRadius:8,padding:12,marginTop:14},requestDetailLabel:{color:'#64748B',fontSize:12,fontWeight:'800',marginTop:2},requestDetailValue:{color:'#0F172A',fontSize:15,fontWeight:'900',marginBottom:7},modalMessage:{color:'#64748B',fontSize:15,fontWeight:'600',textAlign:'center',lineHeight:22,marginTop:14},modalActions:{width:'100%',gap:10,marginTop:22},modalPrimary:{backgroundColor:'#059669',borderRadius:10,paddingVertical:14},modalPrimaryText:{color:'#FFF',fontSize:16,fontWeight:'900',textAlign:'center'},modalSecondary:{backgroundColor:'#EFF6FF',borderRadius:10,paddingVertical:14,borderWidth:1,borderColor:'#BFDBFE'},modalSecondaryText:{color:'#1D4ED8',fontSize:16,fontWeight:'900',textAlign:'center'},
});
